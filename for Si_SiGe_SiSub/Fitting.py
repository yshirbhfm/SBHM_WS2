...
struture_type = 'Si111'
StructProp_sub = bp.BondsPreset(struture_type)
...
def fitness_func_set(OptPar,data, Kin, Ein):
    ...
    def fitness_func(solution, solution_idx):
        ...
        fit_output = s.SBHM(OptPar,StructProp_sub,StructProp_d,x, Kin, Ein)
...
def output_from_solution(solution, OptPar, data, Kin, Ein):
    StructProp_d = parameter_substitution(solution)
    output = s.SBHM(OptPar,StructProp_sub,StructProp_d,data['x'], Kin, Ein)
...
def pygad_fit(OptPar, data, Kin, Ein):
    ...
    initial_population = np.concatenate((
        [np.random.uniform(low=60, high=360/symmetry, size=sol_per_pop )],       #phi0
        [np.random.uniform(low=-a_max/2, high=a_max/2, size=sol_per_pop )],            #au
        [np.random.uniform(low=0, high=0, size=sol_per_pop )],                  #ad
        [np.random.uniform(low=0, high=a_max/5, size=sol_per_pop )],           #ab
        ...
        [np.random.uniform(low=0, high=10, size=sol_per_pop )]),                 #TiltAngle ##跟TiltDirectionAngle 互補
        axis=0).T
    
    gene_space = [
        {'low':60,'high':360/symmetry},       # phi0   ##### None
        {'low':-a_max/2,'high':a_max/2},            # au
        {'low':0,'high':0},                  # ad
        {'low':0,'high':a_max/5},           # ab
        {'low':90,'high':90},                 # defdir theta
        {'low':0,'high':0},                # defdir phi
        {'low':0,'high':0},                 # defRatio
        {'low':0,'high':360},                # TiltDirectionAngle
        {'low':0,'high':10}                   # TiltAngle
        ]
