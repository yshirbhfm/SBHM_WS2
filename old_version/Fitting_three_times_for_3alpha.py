...
#####   
        ...
        return fitness
    return fitness_func
def func_generation_set(solution_2):
    ...
    def func_generation(ga_instance):
        ...
        if ga_instance.generations_completed == 1:
           # print("fdsf") 
           # ga_instance.population[0] = [solution_2[1],solution_2[3],\
           #                                      solution_2[4],solution_2[5],\
           #                                          solution_2[6]]
           print('a')
...
def parameter_substitution(solution,solution_1,solution_2):
    ...
    StructProp_c['phi0'] = solution_2[0]    # deg.
    ...
    TiltProp = {
        'TiltDirectionAngle':  solution[6],    # deg.
...
def pygad3_fit(OptPar, data, solution_1, solution_2):
    ...
    func_generation = func_generation_set(solution_2)
    ...
    initial_population = np.concatenate((
        ...
        [np.random.uniform(low=0.97*solution_2[2], high=1.03*solution_2[2], size=sol_per_pop )],  #ad
        [np.random.uniform(low=0.97*solution_2[3], high=1.03*solution_2[3], size=sol_per_pop )],  #ab
        [np.random.uniform(low=solution_2[4]-5, high=solution_2[4]+5, size=sol_per_pop )],   #defdir_theta  ##跟defdir_phi 互補
        [np.random.uniform(low=solution_2[5]-10, high=solution_2[5]+10, size=sol_per_pop )],   #defdir_phi
        [np.random.uniform(low=0, high=5, size=sol_per_pop )],   #defRatio
        [np.random.uniform(low=solution_2[7]-2, high=solution_2[7]+2, size=sol_per_pop )]),   #TiltDirectionAngle
        # [np.random.uniform(low=solution_2[8]-0.1, high=solution_2[8]+0.1, size=sol_per_pop )]),   #TiltAngle ##跟TiltDirectionAngle 互補
        # [np.random.uniform(low=0, high=360, size=sol_per_pop)]),   #tau
        # [np.random.uniform(low=-20, high=20, size=sol_per_pop)]),    #C
        axis=0).T
    ####
    # initial_population[0] = [solution_2[1],solution_2[3],solution_2[4],solution_2[5],solution_2[6]]                                          \
                                              
    #####
    gene_space = [
        # {'low':solution_1[0],'high':solution_1[0]},   # phi0   ##### None
        {'low':0.97*solution_2[1],'high':1.03*solution_2[1]}, # au
        {'low':0.97*solution_2[2],'high':1.03*solution_2[2]}, # ad
        {'low':0.97*solution_2[3],'high':1.03*solution_2[3]}, # ab
        {'low':solution_2[4]-5,'high':solution_2[4]+5}, # defdir theta
        {'low':solution_2[5]-10,'high':solution_2[5]+10}, # defdir phi
        {'low':0,'high':5}, # defRatio
        {'low':solution_2[7]-2,'high':solution_2[7]+2}  # TiltDirectionAngle
        # {'low':solution_2[8]-0.1,'high':solution_2[8]+0.1} # TiltAngle
        # {'low':0,'high':360} ] #tau
        # None]                   #C
        ]
            ...
            while 1 :
                ...
                if data2['fval'] > 6000 : #太爛就上面重跑
                    ...
                    list_of_solution = [data2['solution'][0],data3['solution'][0],data3['solution'][1],data3['solution'][2],\
                          data3['solution'][3],data3['solution'][4],data3['solution'][5],data3['solution'][6],data2['solution'][8]]
