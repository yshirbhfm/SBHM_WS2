...
def fitness_func_set(OptPar,OptPar_2, data,data_2):
    x = data['x']
    # x, raw, smooth
...
#####   
        ...
        #         +np.mean(((fit_output_2 - desired_output_2)/desired_output)**2)
        #         )######  +0.000011*(90-solution[4])
        #     )
        fitness = np.sqrt(
            1/(
            np.mean(np.square((fit_output - desired_output)/max_d))\
                +np.mean(np.square((fit_output_2 - desired_output_2)/max_d))
                +1e-11)
                )######  +0.000011*(90-solution[4])
...
def output_from_solution(solution, OptPar, OptPar_2, data, data_2):
    StructProp_d = parameter_substitution(solution)
...
def pygad_fit(OptPar, OptPar_2, data, data_2):
    ysm = data['ySmooth']
    fitness_func = fitness_func_set(OptPar, OptPar_2, data, data_2)
    ...
    initial_population = np.concatenate((
        ...
        [np.random.uniform(low=0, high=a_max*1.2, size=sol_per_pop )],  #au
        [np.random.uniform(low=0, high=a_max*1.2, size=sol_per_pop )],  #ad
        [np.random.uniform(low=0, high=a_max/2, size=sol_per_pop )],  #ab
        [np.random.uniform(low=70, high=90, size=sol_per_pop )],   #defdir_theta  ##跟defdir_phi 互補
        ...
        [np.random.uniform(low=0, high=5, size=sol_per_pop )]),   #TiltAngle ##跟TiltDirectionAngle 互補
    ...
    gene_space = [
        ...
        {'low':0,'high':a_max*1.2}, # au
        {'low':0,'high':a_max*1.2}, # ad
        {'low':0,'high':a_max/2}, # ab
    ...
    ysi, ysi_2 = output_from_solution(solution, OptPar, OptPar_2, data, data_2)
    
    return ysi, fval, solution, ysi_2
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        ######   
        if os.path.splitext(gDataOpt['load_file_name'])[0].split('_')[-1] == 'sp':
